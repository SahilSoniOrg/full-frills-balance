...
- **F-02:** Safe-to-Spend converts each nonzero liquid-asset balance to workplace currency at the latest available spot quote before summing the displayed cash total. A missing quote excludes that amount and sets the existing unvalued-input flag; cached duplicate quotes are resolved by newest effective date. The safe-days sign check now reads the simulator's starting balances after conversion. Its status is withheld when a starting balance, budget usage, or forecast input is unvalued; the chart shows no “Safe for…” or “Available” status in that case. History-only FX gaps still mark the dashboard incomplete without obscuring an otherwise complete forward estimate. The dashboard warning is tappable; its popup lists each excluded cash account and amount with the missing current currency pair. Targeted tests verify EUR 10.00 at 1.137 becomes USD 11.37 and unavailable FX is not mislabeled. The iOS Release simulator Detox case passes with EUR 20 and no EUR/USD quote: tapping the warning shows “Euro Wallet: €20.00 was not included” and “No current EUR → USD exchange rate is available”; the safety status is absent. Recording: `artifacts/detox/f02-missing-fx-review/ios-detox-missing-fx.mp4`; screenshots: `artifacts/detox/f02-missing-fx-review/f02-missing-fx-dashboard-warning.png` and `artifacts/detox/f02-missing-fx-review/f02-missing-fx-details-popup.png`. Full unit/Detox suites and large-range performance remain unverified.
