...
Implemented in source; targeted verification complete where noted:
...
- **F-02:** Safe-to-Spend converts each nonzero liquid-asset balance to workplace currency at the latest available spot quote before summing the displayed cash total. A missing quote excludes that amount and sets the existing unvalued-input flag; cached duplicate quotes are resolved by newest effective date. The safe-days sign check now reads the simulator's starting balances after conversion. Its status is withheld when a starting balance, budget usage, or forecast input is unvalued; the chart shows no “Safe for…” or “Available” status in that case. History-only FX gaps still mark the dashboard incomplete without obscuring an otherwise complete forward estimate. Incomplete-FX details now render from an app-shell overlay and open from warnings on Safe to Spend, Budget, classic Reports, Accounts 30-day cash flow, and Reports v2. Safe to Spend lists excluded cash accounts and values using the active screen privacy setting. For itemized missing current cash rates, the sheet can force-refresh the quote and then accept a manually entered spot rate if no quote is available; the rate is persisted as manual and updates the Safe-to-Spend projection. Historical report dates remain isolated from these spot rates. Legacy Reports, Budget, and cash-flow models expose only an aggregate flag, so their popup explains the omission without inventing transaction details. Reports v2 lists missing currency-pair/date quotes; those quotes are deduplicated and are not a one-to-one list of affected journals. Targeted tests verify EUR 10.00 at 1.137 becomes USD 11.37 and unavailable FX is not mislabeled. The iOS Release simulator Detox case confirms the initial EUR 20 missing-rate warning and then taps Refresh rates; this run fetched EUR/USD successfully, closed the details sheet, and restored the Safe-to-Spend status. If refresh cannot supply a quote, the scenario verifies that manual entry is offered. App and E2E TypeScript checks pass. Recording: `artifacts/detox/incomplete-fx-details-container/ios-incomplete-fx-rate-refresh.mp4`; before/after screenshots: `artifacts/detox/incomplete-fx-details-container/incomplete-fx-popup-before-refresh.png` and `artifacts/detox/incomplete-fx-details-container/incomplete-fx-refresh-result.png`. Full unit/Detox suites and large-range performance remain unverified.
...
The audit remains **INCOMPLETE** because repository coverage and depth checks are unresolved. F-01 and F-02 have targeted simulator coverage; full-suite coverage remains unverified.
