...
Status: WP-4 ownership inventory and durable rebuild delivery verified; native-adapter proof remains open
Snapshot: `222679b6`
...
- Schema migrations inspected: versions 2 through 32, including 71 `unsafeExecuteSql` calls plus the v31-to-v32 rebuild-intent migration.
...
Inventory status: **COMPLETE**. No mutation path found by the scoped primitive and caller sweeps remains unclassified. All service/command model preparation, model updates, and private raw reads identified by the ratchet now live behind repository or infrastructure seams; the ratchet is zero and rejects regression. Mandatory rebuild delivery now uses the workplace-scoped `rebuild_intents` table and repository-owned dispatcher. Remaining gates are native-adapter savepoint behavior, native crash/restart proof, and real transaction-owner integration coverage.
...
2. Native SQLite/JSI behavior is not proven for staged-import savepoint rollback or durable outbox recovery across process death; current evidence runs against Loki and mocked adapter boundaries.
...
The durability sweep traced 13 production rebuild-enqueue producers across ledger create/reversal/update/lifecycle, journal bulk operations, account hierarchy/create/merge, and SMS scan. All migrated producers now pass typed intent requests into repository-owned writers, which append `rebuild_intents` in the same database batch. `RebuildQueueService` only dispatches durable rows, recovers expired leases, retains failures for retry, and migrates legacy MMKV rows without deleting them before durable insertion succeeds.
...
| ------------------------------------ | ------------------------------------------------------------------ | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Create one/many                      | Journal feature, account opening/adjustment, planned payments, SMS | `journals`, `transactions`, optional `journal_metadata`, `audit_logs`, `rebuild_intents` | `LedgerWriteService` at `src/services/ledger/ledgerWriteService.ts:86-136` using preparers at `src/data/repositories/journal/journalWriteRepository.ts:64-124` | Audit and rebuild intent are batched; post-commit work is limited to cache/analytics effects | Durable rows roll back together; alternate repository APIs can bypass canonical audit/rebuild policy | `JournalTransactionCoordinator.create/createMany` |
| Update journal and legs              | Journal edit                                                       | same four tables plus `rebuild_intents`                                | Split: `LedgerWriteService` builds audit/rebuild intent at `ledgerWriteService.ts:141-202`; `JournalWriteRepository` owns the writer at `journalWriteRepository.ts:154-267` | Audit and rebuild intent are batched in the repository writer | Durable rows roll back together; named coordinator consolidation remains deferred | `JournalTransactionCoordinator.update` |
| Delete/recover                       | Journal actions                                                    | `journals`, `transactions`, `audit_logs`, `rebuild_intents`             | `LedgerWriteService` at `ledgerWriteService.ts:205-288` | Audit and rebuild intent are batched | Durable rows roll back together | `JournalTransactionCoordinator.delete/recover` |
| Post/revert planned status           | Journal/planned-payment actions                                    | `journal_metadata`, `journals`, `transactions`, `audit_logs`, `rebuild_intents` | `LedgerWriteService` at `ledgerWriteService.ts:291-425` | Audit and rebuild intent are batched | Durable rows roll back together; service/coordinator consolidation remains deferred | `JournalTransactionCoordinator.post/revert` |
| Reversal                             | Journal action                                                     | new reversal `journals`/`transactions`/`audit_logs`, original `journal`, `rebuild_intents` | `LedgerWriteService.createReversalJournal` at `ledgerWriteService.ts` using `JournalWriteRepository.persistReversal` in one writer | Audit, original status, and rebuild intent are in the same batch | Durable rows roll back together; named coordinator remains deferred | Keep `LedgerWriteService.createReversalJournal` as owner until a named coordinator exists |
...
| Bulk duplicate/merge/delete/reassign | Journal list                                                       | `journals`, `transactions`, optional `journal_metadata`, `rebuild_intents` | Repository methods at `journalWriteRepository.ts:489-710`; callers at `journalBulkCommands.ts:98-136`, `291-321`, `448-508`, `555-566` | Rebuild intent is prepared by the repository and batched with the bulk mutation; no audit rows | Durable rows roll back together; merge/delete/reassign still bypass a canonical audit policy | `JournalBulkTransactionCoordinator` |
...
Existing evidence: `src/services/ledger/__tests__/ledgerWriteService.write.test.ts:61-160`, `src/services/ledger/__tests__/ledgerWriteService.lifecycle.test.ts:74-289`, and `src/services/journal/__tests__/journalBulkCommands.test.ts:63-291`. Durable intent insertion and failure retention are covered by the repository/dispatcher tests; native writer rollback remains unproven.

### 2. Account mutations

| Path                             | Initiator                                       | Tables                                                                                                                                                           | Current transaction owner                                                                                                                                                                           | Audit/rebuild behavior                                                                                                                  | Rollback gap                                                                                                                                                                                                  | Recommended owner                                                                                                                                                 |
| -------------------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Create, optional opening balance | Account form, onboarding, system-account lookup | `accounts`, `rebuild_intents`, optional `account_metadata`, `audit_logs`; optionally opening-balances equity plus `journals`/`transactions`/ledger audit | `AccountRepository.persistCreatedAccount` at `src/data/repositories/AccountRepository.ts`; command intent at `src/services/accounts/accountCommands.ts` | Account CREATE audit and rebuild intent are batched with the account; opening journal is a follow-up batch with its own intent | Durable rows roll back together. `getOpeningBalancesAccountId` / balance-correction lookup can still create equity in a separate write when used outside `createAccount` | Keep `AccountRepository.persistCreatedAccount` as owner until a named coordinator exists |
| Update/order/reconcile           | Account forms, list reorder, reconciliation     | `accounts`, `rebuild_intents`, optional `account_metadata`, `audit_logs` | Detail updates use `AccountRepository.update`; hierarchy moves use `AccountTreeTransactionCoordinator` at `src/data/repositories/account/AccountTreeTransactionCoordinator.ts` with receipt restore | Account UPDATE audit and type-change rebuild intent are batched with the row; tree undo audits restore in the same batch | Durable rows roll back together | Keep repository/coordinator ownership |
| Bulk update                      | Account list/hierarchy                          | `accounts`, `account_metadata`, `audit_logs`, optional `rebuild_intents` | Command prepares ops; `persistBatch` at `src/data/repositories/persistBatch.ts` owns the writer | Audit and any type-change rebuild intents are in the same batch | Durable rows roll back together | Keep `persistBatch` as owner |
...
| Merge accounts                   | Account merge action                            | `transactions`, `planned_payments`, `transaction_auto_post_rules`, `budget_scopes`, `budgets`, `accounts`, `account_metadata`, `balance_snapshots`, `audit_logs`, `rebuild_intents` | Command preloads and repositories prepare outside the writer; `persistBatch` commits rewrite + audit + rebuild intent | Audit and durable intent are in the same batch as the rewrite | Durable rows roll back together; named coordinator consolidation is deferred | Keep `persistBatch` as owner until a named coordinator exists |
...
| Account-merge rewrite | Account merge        | same tables plus `rebuild_intents` | Repository preparation in `BudgetRepository`, `PlannedPaymentRepository`, `TransactionWriteRepository`, and existing merge repositories; parent account-merge command commits | Correct parent atomic batch; service expresses merge intent only | Durable intent rolls back with the rewrite; prepare occurs before the writer as required by Watermelon | Keep `persistBatch`/future `AccountMergeTransactionCoordinator` as commit owner |
...
| Post/skip occurrence                   | Planned-payment scheduler/details | ledger tables, `planned_payments`, `rebuild_intents` | `LedgerWriteService.createJournal/postJournal` extra ops, or `persistBatch` for skip-of-existing, at `plannedPaymentOrchestration.ts` | Ledger audit, rebuild intent, and schedule advance are in the same batch | Durable rows roll back together | Keep ledger/`persistBatch` as owner |
...
| Manual journal import                | Journal editor launched from inbox          | `journals`, `transactions`, optional metadata, `audit_logs`, `transaction_inbox_records`, `rebuild_intents` | `JournalService.saveJournalEntry` loads the inbox row and passes link ops into `LedgerWriteService.createJournal` extra ops | Journal audit, rebuild intent, and inbox IMPORTED link are one writer; no independent processed-id ack is required | Durable rows roll back together; retry is idempotent via linked journal / fingerprint | Keep `LedgerWriteService.createJournal` as owner |
| Scan/upsert                          | SMS foreground/background scan              | `transaction_inbox_records`, `rebuild_intents` | `TransactionInboxRepository.persistScanBatch` and `prepareUpsert`; pipeline supplies plain write data and final idempotency recheck | Rebuild intent is batched with the inbox write; analytics runs after the write resolves | Failed batch cannot create an orphaned intent | Keep inbox repository persistScanBatch |
| Auto-post                            | Same scan                                   | inbox plus `journals`, `transactions`, optional metadata, `audit_logs`, `rebuild_intents` | Pipeline supplies journal ops and write data; `TransactionInboxRepository.persistScanBatch` owns model preparation and the writer | Audit and rebuild intent are batched with inbox upsert; analytics runs after commit | Concurrent scans are single-flight per workplace; final write rechecks journals/inbox/processed ids | Keep scan persist as owner |

The initial pipeline lookup at `SmsSyncPipeline.ts:143` and rebuild follow-up queries require explicit workplace predicates. Existing integration coverage now verifies workplace-scoped durable intents and batch composition at `src/services/sms/__tests__/SmsSyncPipeline.integration.test.ts:506-552` and `src/services/__tests__/sms-service-batch.test.ts:94-210`; native adapter failure interleavings remain open.

### 6. Rebuild, integrity, and audit maintenance

| Path                    | Initiator                        | Tables                                                                     | Current transaction owner                                                                                                                                      | Audit/rebuild behavior                                                                               | Rollback gap                                                                                                                                                                    | Recommended owner                                                                                                            |
| ----------------------- | -------------------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Running-balance rebuild | Rebuild dispatcher, integrity, import | `transactions.running_balance`, `balance_snapshots`, `accounts.updated_at` | `AccountingRebuildService` opens writer at `src/services/AccountingRebuildService.ts` and owns prepared ops in `rebuildAccountBalancesInternal` | Derived repair normally has no audit; integrity repair can pass audit `extraOps` into the same batch; dispatcher claims/acks `rebuild_intents` through its repository | Follow-up transaction/snapshot fetches include `workplace_id`. Native crash/restart and multi-process lease behavior remain unproven | Keep `AccountingRebuildService` as owner; keep dispatch state in `RebuildIntentRepository` |
| Integrity force repair  | Settings/bootstrap/import        | same derived tables plus `audit_logs`, optional `rebuild_intents` | Per-account writer at `src/services/integrity/integrityRepair.ts` calls `AccountingRebuildService` with a prepared audit op; final account refresh uses `persistBatch` | Repair audit is in the same batch as the rebuild; any requested follow-up intent is durable | Repair and audit roll back together; native writer behavior remains unproven | Keep integrity + rebuild extra ops; no named coordinator |
...
| ------------------------ | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Stage full import        | Import feature/plugin              | 13 workplace-scoped tables: accounts, journals, transactions, audit logs, budgets/scopes, account/journal metadata, planned payments, SMS rules/inbox, snapshots, rebuild intents | `ImportRepository.batchInsert` at `src/data/repositories/ImportRepository.ts:110-430` | Calculates imported balances, writes `_raw` IDs/status, chunks batches at `:408-425`; rebuild-intent rows are intentionally regenerated by post-import rebuild | Later chunk failure leaves a partial staging workplace; target is protected only if best-effort discard succeeds at `src/services/import/ImportService.ts:127-131` | `ImportStagingRepository` |
...
- Rebuild delivery is a same-writer `rebuild_intents` row; the legacy MMKV queue is migrated once at dispatcher startup. SMS processed IDs are no longer an independent correctness source.
...
- Merge rebuild intents are inserted in the parent `persistBatch`; the dispatcher owns later claim/retry/ack state.
...
- Ledger rebuild intents are inserted with the journal/schedule batch; the dispatcher owns later claim/retry/ack state.
...
- Type-change rebuild intents are inserted with the account mutation; the dispatcher owns later claim/retry/ack state.
...
- Rebuild queue dispatches durable `rebuild_intents` rows with lease recovery, bounded retries, and acknowledgement. Legacy `rebuild_queue_v1`/processing markers are migration input only; native crash/restart proof remains deferred.
...
- Rebuild/repair crash recovery uses the durable `rebuild_intents` outbox and dispatcher lease recovery; legacy MMKV migration is retained for upgrade compatibility.
