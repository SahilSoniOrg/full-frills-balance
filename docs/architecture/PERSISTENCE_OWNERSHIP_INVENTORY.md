...
| ------------------------ | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Publish full import      | Import feature/plugin         | 13 workplace-scoped tables: accounts, journals, transactions, audit logs, budgets/scopes, account/journal metadata, planned payments, SMS rules/inbox, snapshots | `ImportRepository.replaceWorkplace` / `batchInsertNewWorkplace` at `src/data/repositories/ImportRepository.ts:87-145`             | Validates and materializes imported balances in one publication batch; derived balances are regenerated by post-import rebuild | A failed publication rolls back the target batch. After successful new-Workplace publication, the Setup draft and stable operation ID retain retry intent until activation or explicit completion; no second pending-Workplace flag is required | Keep `ImportRepository` as the import-only materialization seam                                                |
| Post-import finalization | Import service                | workplace row, exchange-rate cache, derived balances/snapshots, preferences                                                                                      | `ImportService` sequence at `src/services/import/ImportService.ts:119-223`                                                        | Rebuild and preference failures are warning-only after publication; pending marker enables next-launch pointer repair          | Target ledger has committed; active-pointer and onboarding preference writes can be retried on launch, while disposable rebuild work can be retriggered                                                                                         | Keep finalization in `ImportService` until a durable phase model is required                                   |
...
The current import architecture is: parse, backup, validate, then publish the workplace and its books in one repository-owned batch (`ImportService.ts:73-145`). The target ledger is protected until validation succeeds, and post-publication pointer failures are recoverable through the pending-workplace marker. There is no production staging workplace or raw swap path; native device publication remains outside this environment's proof.

Existing evidence: `src/services/import/__tests__/ImportService.workflow.test.ts`, `src/services/import/__tests__/ImportService.workflow.integration.test.ts`, `src/services/import/__tests__/ImportService.postImportRebuild.test.ts`, and `src/data/repositories/__tests__/ImportRepository.test.ts`. These cover publication rollback, post-publication failure semantics, and the pending-workplace recovery contract; native device execution remains deferred.
...
Owners: `ImportRepository.replaceWorkplace` / `batchInsertNewWorkplace` for publication; `DatabaseRepository` remains the purge/reset seam.

- Keep imported data private until validation completes.
- [x] Publish replacement and first-run import data in one repository-owned database batch.
...
Tests: publication rollback and target preservation are covered in `ImportRepository.test.ts` and `ImportService.workflow.integration.test.ts`; post-publication preference failure and pending-workplace recovery are covered in the ImportService and launch tests. Native device fault injection remains deferred.
