...
| Reversal | Journal action | new reversal `journals`/`transactions`/`audit_logs`, then original `journal` | `LedgerWriteService.createJournal` followed by `JournalWriteRepository.markReversed`; caller at `src/services/journal/journalDomainService.ts:107-150` | First commit audits/rebuilds the reversal; second commit marks original reversed | Failure between `journalDomainService.ts:130` and `141` leaves a reversal journal without the original reversal link/status | `JournalTransactionCoordinator.reverse` |
...
| Path | Initiator | Tables | Current transaction owner | Audit/rebuild behavior | Rollback gap | Recommended owner |
| --- | --- | --- | --- | --- | --- | --- |
| Create, optional opening balance | Account form, onboarding, system-account lookup | `accounts`, optional `account_metadata`; then `audit_logs`; optionally `journals`, `transactions`, ledger audit | `AccountRepository.create` at `src/data/repositories/AccountRepository.ts:407-436`, then command sequencing at `src/services/accounts/accountCommands.ts:81-148` | Account audit is separate at `accountCommands.ts:95-117`; opening-balance ledger is another commit | Account can exist unaudited and without its requested opening balance; retries can observe a half-initialized aggregate | `AccountTransactionCoordinator.createWithOpeningBalance` |
