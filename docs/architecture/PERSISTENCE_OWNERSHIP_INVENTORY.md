...
Status: WP-4 design inventory complete; ownership migrations and enforcement verified
Snapshot: `136b2777`
...
Inventory status: **COMPLETE**. No mutation path found by the scoped primitive and caller sweeps remains unclassified. All service/command model preparation, model updates, and private raw reads identified by the ratchet now live behind repository or infrastructure seams; the ratchet is zero and rejects regression. Remaining implementation gates are durable rebuild delivery, native-adapter savepoint behavior, and real transaction-owner integration coverage. The migration plan treats each guarantee as absent until its stronger test exists.
...
| Merge accounts                   | Account merge action                            | `transactions`, `planned_payments`, `transaction_auto_post_rules`, `budget_scopes`, `budgets`, `accounts`, `account_metadata`, `balance_snapshots`, `audit_logs` | Command preloads and repositories prepare outside the writer; `persistBatch` commits rewrite + audit; rebuild callback runs after resolve | Audit is in the same batch as the rewrite; rebuild enqueue is post-commit                                                   | Durable rows roll back together; rebuild intent is still not durable. Named coordinator consolidation is deferred                         | Keep `persistBatch` as owner until a named coordinator exists; next gap is durable rebuild/outbox                           |
...
| Account-merge rewrite | Account merge        | same tables                | Repository preparation in `BudgetRepository`, `PlannedPaymentRepository`, `TransactionWriteRepository`, and existing merge repositories; parent account-merge command commits | Correct parent atomic batch; service expresses merge intent only | Rebuild intent remains non-durable; prepare occurs before the writer as required by Watermelon | Keep `persistBatch`/future `AccountMergeTransactionCoordinator` as commit owner |
