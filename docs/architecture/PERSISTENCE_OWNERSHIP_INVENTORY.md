...
Status: WP-4 ownership inventory complete; native device proof remains open; Workplace transition follow-up is tracked separately
Snapshot: historical baseline `0ff90512`; see `ARCHITECTURE_AUDIT.md` for current follow-up status
...
1. Import publication now uses repository-owned Watermelon batches; rollback and target preservation are covered by repository/workflow tests, while native SQLite/JSI behavior remains unproved.
2. Post-publication preference failures are warning-only and recover through the pending-workplace marker. Pending MMKV rebuild work may be lost on process death by design.
...
Services should validate and express domain intent. Repositories should load records and prepare table-local operations. Existing domain services own their bounded commits; do not add a generic transaction-coordinator layer without a concrete invariant that needs it. External or derived effects should run through an explicit after-commit contract. Disposable projection triggers may use MMKV; only a future mandatory, non-reconstructible workflow would justify a durable outbox.

```text
feature / background trigger
  -> domain service (validate and build intent)
...
| Post/skip occurrence                   | Planned-payment scheduler/details | ledger tables, `planned_payments`                        | `settlePlannedOccurrence` in `plannedOccurrenceSettlement.ts` inside one `runAccountingWriteSession`                                                               | Payment re-read, occurrence lookup, journal action, and schedule advance share one writer; rebuild is enqueued after commit | Business rows roll back together; derived projection may remain stale until retriggered         | Keep the accounting write session as owner                                      |
| Generate due horizon                   | Bootstrap/scheduler               | ledger tables, `planned_payments`                        | Loop at `plannedPaymentOrchestration.ts`; one `settlePlannedOccurrence` session per occurrence, each payment isolated by try/catch                                  | Occurrence lookup (including `ORIGINAL_PLANNED_DATE` for posted journals) runs inside the writer; already-settled days only advance the schedule | A failure on one payment leaves other payments processing                                       | Keep per-occurrence sessions                                                    |
...
| ------------------------- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Publish full restore      | Setup restore journey         | 13 workplace-scoped tables: accounts, journals, transactions, audit logs, budgets/scopes, account/journal metadata, planned payments, SMS rules/inbox, snapshots | `publishRestore` uses `ImportRepository.batchInsertNewWorkplace`                                                                  | Validates before consent, then atomically creates a new Workplace; derived balances and exchange rates are refreshed afterward | A failed publication rolls back the new Workplace batch. Stable operation IDs and source fingerprints make retry idempotent; existing Workplaces are never replaced                      | Keep `restore.ts` as the restore seam and `ImportRepository` as its private materialization owner              |
| Post-restore finalization | Restore module and Setup      | exchange-rate cache, derived balances/snapshots, workplace preferences, active pointer                                                                           | `publishRestore` owns book verification; Setup finishers own appearance, device registration, activation, and navigation          | Rebuild, rate, and integrity failures become warnings after publication                                                        | Publication is durable before Setup activates it. The persisted handoff allows finish to resume, while operation-owned inactive books can be cleaned up after an interrupted publication | Keep book effects in restore; keep user/device effects in Setup                                                |
| Incremental apply         | Removed; no production caller | accounts, journals, transactions, audit logs                                                                                                                     | Deleted `ImportRepository.applyChanges`; full publication is the only supported import write path                                 | N/A                                                                                                                            | Keeping an unowned mutation API would drift from full import and bypass publication guarantees                                                                                           | Reintroduce only with a concrete product requirement and workflow tests                                        |
| Create/delete workplace   | Onboarding/settings           | `workplaces`, system accounts, optional initial ledger; all workplace tables on delete                                                                           | `WorkplaceService` owns persistence; `LaunchCoordinator` owns active-workplace transitions, cache eviction, and routing           | Analytics/preferences follow persistence                                                                                       | Deletion is irreversible; cleanup after the database commit is best-effort and returns committed with warnings when recovery is needed                                                   | Keep the two existing owners; add a third coordinator only if a new invariant requires it                      |
...
The current restore architecture is: read and parse every candidate, validate, let the user select any or all validated Workplaces, review, then publish each selected Workplace and its books in a repository-owned batch. Unselected candidates never reach persistence, and existing Workplaces are never replaced. Setup owns selection, consent, user/device preferences, activation, and navigation; the restore module owns imported books and workplace-scoped preferences.

Existing evidence: `src/services/import/__tests__/restoreBoundary.test.ts`, `src/services/import/__tests__/validateImportedData.test.ts`, the Setup coordinator tests, and `src/data/repositories/__tests__/ImportRepository.test.ts`. These cover validation, idempotent publication, review-before-publication ordering, and atomic new-Workplace writes; native device fault injection remains deferred.
...
- `DatabaseRepository`: approved privileged lifecycle seam for purge/reset. Import publication no longer uses a raw staged swap.
...
- Forced workplace identity: `WorkplaceRepository.ts:26-35`. Restrict to import publication APIs.
...
- Due-horizon generate checks occurrence existence inside the serialized writer, so concurrent runs cannot both create the same occurrence.
...
### Slice 6 — P1 import publication

Owners: the restore module and `ImportRepository.batchInsertNewWorkplace` for publication; `DatabaseRepository` remains the purge/reset seam.

- Keep imported data private until validation completes.
- [x] Publish restored data as a new Workplace in one repository-owned database batch.
- [x] Treat post-publication pointer/preference failure as recoverable rather than a failed import.
- Durable phase resume across process death is deferred; the pending-workplace marker covers pointer repair.

Tests: publication retry and validation are covered at the restore interface; atomic materialization is covered in `ImportRepository.test.ts`; Setup and launch tests cover handoff recovery and activation. Native device fault injection remains deferred.
...
- Import publication rollback is covered by repository and workflow tests; native SQLite/JSI device execution remains deferred.
