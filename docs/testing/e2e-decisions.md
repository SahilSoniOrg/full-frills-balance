...
Native Detox is the **authoritative** UI signal for SQLite, widgets, and dev-client behavior. Playwright covers overlapping journeys on the static web export (LokiJS adapter).

## Platforms

- **iOS simulator first** — `iPhone 17`, configuration `ios.sim.debug` in `.detoxrc.js`
...
```bash
bun run e2e:build:ios      # Xcode simulator build for Detox
bun run e2e:test:ios       # All critical specs under e2e/specs
