...
describe('AccountingService', () => {
  ...
  describe('calculateNewBalance', () => {
    it('should correctly calculate increase for Asset Debit', () => {
      // 100 + (50 * 1) = 150
      expect(service.calculateNewBalance(100, 50, AccountType.ASSET, TransactionType.DEBIT)).toBe(
        150,
      );
    });
    ...
    it('should correctly calculate decrease for Asset Credit', () => {
      // 100 + (50 * -1) = 50
      expect(service.calculateNewBalance(100, 50, AccountType.ASSET, TransactionType.CREDIT)).toBe(
        50,
      );
    });
    ...
    it('should handle precision', () => {
      expect(
        service.calculateNewBalance(100.12, 0.005, AccountType.ASSET, TransactionType.DEBIT, 2),
      ).toBe(100.13);
    });
  });
