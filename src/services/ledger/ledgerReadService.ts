  ...
  observeEnrichedForAccounts(
    ...
    const journalIds$ = transactions$.pipe(
      map((transactions: Transaction[]) =>
        Array.from(new Set(transactions.map(t => t.journalId))).sort(),
      ),
      distinctUntilChanged((a, b) => a.length === b.length && a.every((id, idx) => id === b[idx])),
    );
    ...
    return combineLatest([transactions$, journals$, allAccounts$, allJournalTransactions$]).pipe(
      ...
      map(([transactions, journals, allAccounts, allJournalTransactions]) => {
        ...
        for (const tx of allJournalTransactions as Transaction[]) {
        ...
        return transactions.map(tx => {
          ...
          const isIncrease = isBalanceIncrease(
            txAccount?.accountType as any,
            tx.transactionType as any,
          );

          // Optimized counter-party lookup
          const journalTransactions = transactionsByJournal.get(tx.journalId) || [];
          const counterPartyTransactions = journalTransactions.filter(
            (jtx: Transaction) => jtx.transactionType !== tx.transactionType,
          );

          const counterAccounts = counterPartyTransactions.map((ctx: Transaction) => {
          ...
          return {
            ...
            icon: txAccount?.icon,
            runningBalance:
              rootAccountIds.length === 1 && rootAccountIds.includes(tx.accountId)
                ? tx.runningBalance
                : undefined,
