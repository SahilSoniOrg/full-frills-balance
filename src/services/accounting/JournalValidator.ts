...
export class JournalValidator {
   ...
   */
  static validate(lines: JournalLineInput[], baseCurrency: string): ValidationResult {
    const errors: string[] = [];
    ...
    if (!JournalCalculator.isBalanced(lines, baseCurrency)) {
    ...
    if (
      lines.some(l => l.exchangeRate !== undefined && Number(l.exchangeRate) <= MIN_EXCHANGE_RATE)
    ) {
