...
export interface TransactionShareOptions {
  title?: string;
  filename?: string;
  includeTime?: boolean;
  sort?: 'asc' | 'desc' | 'none';
...
export class TransactionShareProvider implements ShareProvider {
  ...
  constructor(
    ...
    private options: TransactionShareOptions = {},
  ) {
    this.title = options.title || 'Transaction Report';
    // Filename uniqueness is handled by SharingService
    this.filename = options.filename || 'transactions-report';
  }

  getContent(format: ShareFormat): string {
    if (this.transactions.length === 0) return 'No transactions to share';
  ...
  private formatAsText(
    transactions: ShareableTransaction[],
    options: TransactionShareOptions,
    generatedAt: number,
  ): string {
    const lines: string[] = [];
    ...
    if (transactions.length === 0) {
      if (defaultCurrency) {
        ...
        lines.push('No transactions available for this period');
      }
    }
  ...
  private formatAsCSV(transactions: ShareableTransaction[], _generatedAt: number): string {
  ...
  private formatAsMarkdown(transactions: ShareableTransaction[], generatedAt: number): string {
    const lines: string[] = [];
