...
export class CanonicalImportBuilder {
  ...
  private synthesizeTransactions(
    ...
    for (const tx of this.rawTransactions) {
      ...
      const txCurrency = tx.currencyCode || this.defaultCurrency;
      const date = tx.date ?? Date.now();
      const amount = Math.abs(tx.amount);

      if (tx.type === 'TRANSFER') {
        const sourceId = tx.sourceAccountId ? accountMap.get(tx.sourceAccountId) : undefined;
        const destId = tx.targetAccountId ? accountMap.get(tx.targetAccountId) : undefined;
        ...
        const sourceCurrency = accountCurrencyMap.get(tx.sourceAccountId) || txCurrency;
        const destCurrency =
          (tx.targetAccountId && accountCurrencyMap.get(tx.targetAccountId)) || txCurrency;
        ...
        const assetAccountId = tx.sourceAccountId ? accountMap.get(tx.sourceAccountId) : undefined;
        if (!assetAccountId) {
          issues.push({
            severity: 'error',
            entity: 'transaction',
        ...
        if (tx.isOpeningBalance) {
          categoryAccountId = this.getOrCreateSystemEquityAccount(
            true,
            txCurrency,
            categoryAccountMap,
            canonicalAccounts,
          );
        } else if (tx.categoryId && categoryAccountMap.has(`${tx.categoryId}:::${txCurrency}`)) {
          categoryAccountId = categoryAccountMap.get(`${tx.categoryId}:::${txCurrency}`)!;
        } else {
          categoryAccountId = this.getOrCreateUnknownCategory(
            true,
            txCurrency,
            categoryAccountMap,
            canonicalAccounts,
