...
const mapFrequency = (reoccurrence: number | null): string => {
    switch (reoccurrence) {
        ...
        case 0: return 'MONTHLY'; // Default custom to Monthly for now
...
export const cashewPlugin: ImportPlugin = {
    ...
    detect(context: ImportFileContext): boolean {
        // 1. Check filename
        const isCashewExt = context.name.toLowerCase().includes('cashew') ||
    ...
    async import(context: ImportFileContext, onProgress?: (message: string, progress: number) => void): Promise<ImportStats> {
        ...
        try {
            ...
            // Fetch Data
            const wallets: CashewWallet[] = await db.getAllAsync<CashewWallet>('SELECT wallet_pk, name, colour, icon_name, currency, pinned, "order", budget_transaction_filters, member_transaction_filters, shared_key, shared_owner_member, shared_date_updated, shared_members, shared_all_members_ever, is_absolute_spending_limit FROM wallets');
            const categories: CashewCategory[] = await db.getAllAsync<CashewCategory>('SELECT category_pk, name, colour, icon_name, income, main_category_pk FROM categories');
            ...
            const budgets: CashewBudget[] = await db.getAllAsync('SELECT budget_pk, name, amount, start_date, wallet_fks, category_fks, income, archived FROM budgets');
            const scannerTemplates: CashewScannerTemplate[] = await db.getAllAsync('SELECT scanner_template_pk, date_created, date_time_modified, template_name, contains, title_transaction_before, title_transaction_after, amount_transaction_before, amount_transaction_after, default_category_fk, wallet_fk, ignore FROM scanner_templates');
            const objectives: CashewObjective[] = await db.getAllAsync('SELECT objective_pk, type, name, amount, "order", colour, date_created, end_date, date_time_modified, icon_name, emoji_icon_name, income, pinned, archived, wallet_fk FROM objectives');
            const appSettings: CashewAppSetting[] = await db.getAllAsync('SELECT settings_pk, settings_json, date_updated FROM app_settings LIMIT 1');
            ...
            for (const t of allTransactions) {
                ...
                const walletCurrency = walletCurrencies.get(t.wallet_fk) || preferences.defaultCurrencyCode || 'USD';
                ...
                if (t.category_fk === correctionCategoryPk && t.paired_transaction_fk) {
                    ...
                    if (pair) {
                        const fromAccId = t.amount < 0 ? accountsMap.get(t.wallet_fk) : accountsMap.get(pair.wallet_fk);
                        const toAccId = t.amount > 0 ? accountsMap.get(t.wallet_fk) : accountsMap.get(pair.wallet_fk);
            ...
            if (appSettings.length > 0 && data.exchangeRates) {
                try {
                    ...
                    logger.warn('[CashewPlugin] Failed to parse app settings for exchange rates', { error: e });
                }
            }
            ...
            throw new Error(`Cashew import failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
};
