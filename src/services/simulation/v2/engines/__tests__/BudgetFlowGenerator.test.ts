...
describe('BudgetFlowGenerator', () => {
  ...
  const budgets = [{ id: 'b1', name: 'Groceries', amount: 300, assetAccountIds: 'acc-1' }];
  const usages = [{ budgetId: 'b1', spent: 0, remaining: 300 }];
  const budgetCategoryMap = new Map([['b1', new Set(['cat-1'])]]);
  ...
  it('respects assetAccountIds and distributes burn', () => {
    ...
    const flows = BudgetFlowGenerator.generate(
      ...
      multiAccountBudget,
      [{ budgetId: 'b2', spent: 0, remaining: 100 }] as any,
      10,
      30,
      new Map([['b2', new Set()]]),
    );
    ...
    expect(flows.filter(f => f.accountId === 'acc-1')).toHaveLength(30); // because simulationDays = 30
    ...
    const day0Acc1 = flows.find(f => f.dayOffset === 0 && f.accountId === 'acc-1');
    expect(day0Acc1?.amount).toBe(5);
  });
});
