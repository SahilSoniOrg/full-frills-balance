...
export const hasHardwareAsync = async () => {
  if (Platform.OS === 'web') {
    const available = typeof window !== 'undefined' && window.PublicKeyCredential !== undefined;
...
export const enrollAsync = async (options: LocalAuthentication.LocalAuthenticationOptions) => {
  if (Platform.OS === 'web') {
    try {
      ...
      logger.error('WebAuthn enrollment error:', error);
...
export const authenticateAsync = async (
  ...
  if (Platform.OS === 'web') {
    try {
      ...
      logger.error('WebAuthn auth error:', error);
      return { success: false, error: String(error) };
    }
  }
