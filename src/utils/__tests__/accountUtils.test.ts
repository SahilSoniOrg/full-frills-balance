...
describe('accountUtils', () => {
  ...
  describe('groupAccountsByType', () => {
    ...
    it('should handle case-insensitive account types', () => {
      const accountsWithLowerType = [{ id: '1', name: 'Cash', accountType: 'asset' }];
  ...
  describe('getAccountSections', () => {
    it('should return sections in standard order', () => {
      ...
      expect(sections[2].title).toBe('Income Categories');
      expect(sections[3].title).toBe('Expense Categories');
    });
    ...
    it('should only include sections with accounts', () => {
      const partialAccounts = [{ id: '1', name: 'Salary', accountType: AccountType.INCOME }];
      const sections = getAccountSections(partialAccounts as any);

      expect(sections).toHaveLength(1);
      expect(sections[0].title).toBe('Income Categories');
    });
