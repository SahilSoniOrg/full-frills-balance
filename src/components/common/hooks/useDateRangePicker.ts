...
export function useDateRangePicker({
  ...
  const monthList = useMemo(() => {
    const list = [];
    const today = dayjs();
    for (let i = INITIAL_MONTH_INDEX; i > 0; i--) {
  ...
  const handleSelectCustom = useCallback(() => {
    setDraftFilter({
      type: 'CUSTOM',
      startDate: customRange.startDate ? customRange.startDate.valueOf() : 0,
      endDate: customRange.endDate ? customRange.endDate.valueOf() : 0,
    });
  }, [customRange]);
  ...
  const handleApply = useCallback(() => {
    ...
    if (
      ...
      range = {
        ...mRange,
        label: `${dayjs().month(draftFilter.month).format('MMM')} ${draftFilter.year}`,
      };
