...
/** Node amount labels: useMoneyFormat + SvgText (SVG children). Host under PrivacyScopeProvider. */
...
export const SankeyChart: React.FC<SankeyChartProps> = ({
  ...
  const layout = useMemo(() => {
    ...
    nodes.forEach(n => {
      if (n.id === 'total_income') {
        nodeValue[n.id] = links
          .filter(l => l.target === 'total_income')
    ...
    columnNodes.forEach((col, colIdx) => {
      ...
      const colTotalValueWithSpacing =
        col.reduce((sum, n) => sum + nodeValue[n.id], 0) * scale + (col.length - 1) * NODE_SPACING;
  ...
  }, [nodes, links, height, NODE_SPACING]);
    ...
    <View style={{ height, width: CHART_WIDTH }}>
      <Svg height={height} width={CHART_WIDTH}>
        ...
        {formattedLinks.map((l, i) => {
          const sourceCol = nodes.find(
            n =>
              n.id === l.source &&
              links.some(link => link.target === 'total_income' && link.source === n.id),
          )
            ? 0
            : 1;
        ...
        {nodes.map(n => {
          const col =
            n.id === 'total_income'
              ? 1
              : links.some(l => l.target === 'total_income' && l.source === n.id)
                ? 0
                : 2;
          const x = colX[col];
          const y = nodeY[n.id];
          ...
          return (
            <React.Fragment key={n.id}>
              <Rect x={x} y={y} width={NODE_WIDTH} height={h} fill={nodeColor} rx={2} />
              <SvgText
                x={
                  isTotal
                    ? x + NODE_WIDTH + LABEL_OFFSET
                    : isRightSide
                      ? x - LABEL_OFFSET
                      : x + NODE_WIDTH + LABEL_OFFSET
                }
