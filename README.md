...
Existing debt around unscoped raw queries, presentation imports of database models, direct database writes outside persistence seams, and service/command model persistence access is explicitly baselined. The architecture check rejects new occurrences and stale baseline entries; it does not claim those boundaries are fully clean until the baselines reach zero.
